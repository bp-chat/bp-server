...
pub fn build(b: *std.Build) void {
    ...
    const test_client_exe = b.addExecutable(.{
        .name = "bp-test-client",
        .root_source_file = b.path("src/test-client.zig"),
        .target = target,
        .optimize = optimize,
    });
    ...
    const run_main_step = b.step("run", "Run the server");
    run_main_step.dependOn(&run_main.step);
    const run_test_client_step = b.step("run-client", "Run test client");
    run_test_client_step.dependOn(&run_test_client.step);

    const exe_main_unit_tests = b.addTest(.{
        .root_source_file = b.path("src/main.zig"),
        .target = target,
        .optimize = optimize,
    });
    const exe_test_client_unit_tests = b.addTest(.{
        .root_source_file = b.path("src/test-client.zig"),
        .target = target,
        .optimize = optimize,
    });

    const run_main_unit_tests = b.addRunArtifact(exe_main_unit_tests);
    const run_test_client_unit_tests = b.addRunArtifact(exe_test_client_unit_tests);
    ...
    test_step.dependOn(&run_test_client_unit_tests.step);
}
